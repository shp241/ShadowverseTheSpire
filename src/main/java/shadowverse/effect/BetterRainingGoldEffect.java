...
public class BetterRainingGoldEffect extends AbstractGameEffect {
    private int amount;
    private int min;
    private int max;
    ...
    public BetterRainingGoldEffect(int amount) {
    ...
    public BetterRainingGoldEffect(int amount, boolean centerOnPlayer) {
        this(amount);
    ...
    public void update() {
        ...
        if (this.staggerTimer < 0.0F) {
            ...
            for(int i = 0; i < goldToSpawn; ++i) {
                AbstractDungeon.effectsQueue.add(new BetterTouchPickupGold(this.playerCentered));
            }
